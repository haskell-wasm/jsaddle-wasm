...
runHelper entryPoint sendOutgoingMessage onIncomingMessage = do
  ...
  where
    ...
    decodeResults = A.throwDecodeStrictText <=< jsStringToText
...
-- Conversion JSString <-> ByteString/Text
...
jsStringToText :: JSString -> IO T.Text
jsStringToText s = do
  len_max <- (* 3) <$> js_stringLength s
  mba@(MutableByteArray mba#) <- newByteArray len_max
  len <- js_encodeInto s mba# len_max
